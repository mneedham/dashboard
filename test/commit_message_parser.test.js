...
Pair = commit_message_parser.Pair;

module.exports = {
    'parse()': function(){
        assert.ok(Pair({ person1: "Ken", person2 : "Alex"}).equals(commit_message_parser.parse("Ken/Alex #266 added empty authors column so that subjects appear in 3rd column")));
        assert.ok(Pair({ person1: "Alinoor", person2 : "Mark"}).equals(commit_message_parser.parse("alinoor/mark #380 more stuff this time for chapter title")));
        assert.ok(Pair({ person1: "Mark", person2 :  "Suzuki"}).equals(commit_message_parser.parse("mark,suzuki #232 fix merge")));
	    assert.ok(Pair({ person1: "Pat", person2 : "Sebastian"}).equals(commit_message_parser.parse("pat, sebastian: removing unnecessary line in dev properties.")));
	    assert.ok(Pair({ person1: "Charles", person2 :  "Mark"}).equals(commit_message_parser.parse("charles/mark: adios to play, hello scalatra")));
    },
    'pairs()' : function() {
	   assert.eql({"Fri Jun 03 2011" : [Pair({person1 : "Alex", person2 : "Charles"})]}, commit_message_parser.pairs([{message:"Alex/Charles #0 Added functional ML test", date:"Fri Jun 03 2011"}]));  
