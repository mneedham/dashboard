...
var aliases = {"lix":"liz", "chrisj":"chris", "mustaq":"mushtaq", "mike":"michael", "micheal":"michael", "ducan":"duncan", "rw":"rob", "ld":"liz"};
...
function hasPair(commitMessage) {
	var matches = pairRegex.test(commitMessage);
	if(!matches) {
...
exports.pairsFor = function(commits, person) {
	...
	for(var date in pairs) {
	  if(pairs.hasOwnProperty(date) && pairs[date].length > 0) {
	    ...
	    var people = [];
	    peoplePairedWith[date].forEach(function(pair) {
		  people.push(pair.otherPerson(person))  
	    });
