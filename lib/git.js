...
var pairRegex = /^\[?([\w-]+)[ ]?[^\/, ]*(?:[\/,][ ]?|:)([\w-]+)\]?[^\/]*[\s:]/;
var bannedWordsRegex = /(Revert|Adding)/
...
exports.pairs = function(commits) {
	...
	for(var date in pairs) {
	  if(pairs.hasOwnProperty(date)) {
		pairs[date].uniqueEquals().forEach(function (uniquePair) {
		  uniquePair.commits = 0;
		  pairs[date].forEach(function(pair) {
		    if(pair.equals(uniquePair)) {
				uniquePair.commits += 1;
			}
		  });
		  if(!newPairs[date]) newPairs[date] = [];
		  newPairs[date].push(uniquePair);
		});
	  }
	}
